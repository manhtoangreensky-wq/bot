...
| Mobile hash navigation focus | Closing the menu after every navigation link could leave keyboard focus on a link that becomes hidden. | Same-page hash links now close the menu and move focus to the visible destination heading after the browser updates the hash; modifier/new-tab clicks keep their native behavior. |
