...
- [x] Deploy the diagnostic and query only existing job `#A5F1F9CB0A`; diagnostic PR `#1017` merged as `1705db02cc79aef28f61875e9b8fbfaae20b86eb`, deploy run `34803158214` succeeded, and the read-only result exposed `acoustic_word_time_invalid` with `629` words / `181000ms`. No create/retry/rearm/upload/Confirm/charge/provider action occurred.
- [x] Write exactly one RED reproducing the measured failure code; make one minimal Auto Multi-only correction. RED: `1 failed` at `validate_word_timeline` when declared duration exceeds measured PCM; correction clamps only bounded EOF-crossing words (gap `<=0.5s`, word span `<=2.5s`) and leaves outside-media starts/long spans fail-closed. No threshold or fallback change.
- [x] Run the Auto Multi and locked Auto 2 comparators. Measured: ONNX `136 passed`, blackbox `68 passed`, continuity `7 passed`, recovery `162 passed`, provider-fallback `56 passed`, exact-two selector `3 passed, 284 deselected`; changed service/test compile and diff-check exit `0`.
