...
| Terminal word có đuôi timestamp vượt bound thì phải bỏ toàn bộ timeline | Job `#B1CED79982` có `629` words và reject ở index `628`; nếu terminal word bắt đầu trong media, provider duration nguyên đã xác minh và span `<=2.5s`, hệ thống kẹp `end` về media duration; word giữa/ngoài media vẫn bị chặn | ✅ Guard theo ngữ nghĩa, không theo fixture |
