...
| Terminal word có đuôi timestamp vượt bound thì phải bỏ toàn bộ timeline | Job `#B1CED79982` và `#E1B3795806` đều có `629` words, reject ở terminal index `628`; job sau chứng minh duration làm tròn `181s` có thể bằng provider metadata `181s`. Nếu terminal word bắt đầu trong media, provider duration nguyên đã xác minh trong biên `<0.5s` và span `<=2.5s`, hệ thống kẹp `end` cả khi hai duration bằng nhau; word giữa/ngoài media vẫn bị chặn | ✅ Guard theo ngữ nghĩa, không theo fixture |
