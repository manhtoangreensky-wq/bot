...
| `SD-MS-L09` | Private pipeline context + same-job source rehydrate | Bốn field `_pipeline_workspace`, `_pipeline_saved_source_path`, `_pipeline_source_bytes_override`, `_pipeline_source_content_type_override` còn nguyên qua prepare/translation và tới classifier/TTS/mux. Nếu source đã cleanup, chỉ stored Telegram `file_id` khớp job-key được tải qua transport hiện hành; exact size/MIME/SHA phải PASS trước atomic write và trước CAS. Existing wrong hash, file-id mismatch, download hash mismatch, marker duplicate và CAS loser đều không ghi DB/không overwrite source |
