...
| `SD-MS-L15` | Existing validated MP4 delivery-only recovery | Khi pipeline đã có MP4 validated nhưng recovery message thiếu media methods, chỉ được CAS đúng một marker delivery-only, giữ cùng job/artifact, gửi đúng 1 Telegram video rồi đúng 1 receipt. Nếu root failure snapshot mất job-key/selection/marker thì chỉ reconstruct từ DB + exact manifest + cache + sidecar + SRT content hash; mọi mismatch fail-closed. Cấm replay ASR/translation/TTS/mux/provider, cấm job mới, cấm SRT/audio/document companion; timeout/unknown send phải fail-closed để tránh gửi trùng |
