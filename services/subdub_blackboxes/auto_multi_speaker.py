...
AUTO_MULTI_FAILURE_STAGES = frozenset({
    "entry_validation",
    "voice_pool_capacity",
    "prepare_subtitles",
    "post_prepare_gate",
...
AUTO_MULTI_FAILURE_CODES = frozenset({
...
def _bounded_auto_multi_failure(
    error: Exception,
    *,
    stage: str,
    code: str = "",
    ...
    if bounded_stage not in AUTO_MULTI_FAILURE_STAGES:
    ...
    if bounded_code not in AUTO_MULTI_FAILURE_CODES:
...
def _multi_manual_required_result(
    ...
    *,
    ...
    bounded_failure = _bounded_auto_multi_failure(
        error,
        stage=stage,
        code=code,
    )
...
async def _run_multi_speaker_preflight(
    ...
    *,
    ...
    try:
        ...
        failure_stage = "post_prepare_gate"
        ...
        failure_stage = "classifier_inputs"
        labels, ranges_by_speaker = auto_speaker._validated_classifier_inputs(
            prepared
        )
        failure_stage = "speaker_classification"
...
def _annotate_multi_prepared_assignments(
    ...
    for raw_segment in raw_output:
        ...
        if (
            abs(out_start - src_start) > MULTI_CUE_ROUNDING_TOLERANCE_SECONDS
            or abs(out_end - src_end) > MULTI_CUE_ROUNDING_TOLERANCE_SECONDS
        ):
            raise speaker_cast.AutoCastUnavailable()
...
def _validated_multi_assigned_segments(
    ...
    for raw_segment in raw_segments:
        ...
        if (
            abs(seg_start - src_start) > MULTI_CUE_ROUNDING_TOLERANCE_SECONDS
            or abs(seg_end - src_end) > MULTI_CUE_ROUNDING_TOLERANCE_SECONDS
        ):
            raise speaker_cast.AutoCastUnavailable()
    ...
    try:
        ...
        if any(
            ...
            return _multi_manual_required_result(
                current,
                speaker_cast.AutoCastManualRequired(),
                stage="voice_pool_capacity",
                code="voice_pool_capacity_insufficient",
            )
