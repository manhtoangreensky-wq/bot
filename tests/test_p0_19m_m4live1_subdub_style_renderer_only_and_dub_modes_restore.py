...
def test_m4live1_subtitle_margin_near_bottom_edge():
    ...
    ass = bot.subdub_generate_ass_from_srt(VALID_SRT, _style_state())
    fields = _style_fields(ass)
    assert 1 <= style["subtitle_margin_v_after"] <= 3
...
def _is_storage5_scope(changed: set[str]) -> bool:
    allowed = {
        ...
        "scripts/install_vps_storage_cleanup_timer.sh",
        "scripts/install_vps_storage_maintenance_timers.sh",
        "scripts/vps/cleanup_worker_temp.sh",
        "scripts/vps_storage_cleanup.sh",
        ...
        "tests/test_p0_storage5_backend_isolation_and_path_safety.py",
        "tests/test_p0_storage5_job_cleanup_daily_sweep.py",
        "tests/test_p0_storage5_weekly_backup_retention.py",
        "tests/test_p0_storage5_systemd_timer_contract.py",
