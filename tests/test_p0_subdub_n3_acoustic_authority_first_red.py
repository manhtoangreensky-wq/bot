...
def _compute_dual_gate_evidence(cues: list[dict]) -> dict:
    """Replay the dual-gate logic offline to extract all 6 mandatory report values.

    Returns dict with: raw_dominance, median_margin, margin_gate_result,
    computed_confidence, register_confidence_gate_result, final_cast_result.
    """
    from services import subdub_multi_speaker_gender_onnx as multi_onnx
    male_votes = 0
    female_votes = 0
    ...
    for c in cues:
        m, f = float(c["male_score"]), float(c["female_score"])
        male_votes += int(m > f)
        female_votes += int(f > m)
        total = m + f
        score_margins.append(abs(m - f) / total if total > 0.0 else 0.0)
    winner_votes = max(male_votes, female_votes)
    raw_dominance = winner_votes / len(cues)
    median_margin = float(sorted(score_margins)[len(score_margins) // 2])
    margin_gate = "PASS" if median_margin >= multi_onnx.MIN_PANN_SCORE_MARGIN else "FAIL"
    if margin_gate == "PASS":
        confidence = round(
            float(min(1.0, (0.25 + 0.75 * raw_dominance) * min(1.0, 0.50 + median_margin))),
            6,
        )
    else:
        confidence = None
    if confidence is not None:
        conf_gate = "PASS" if confidence >= speaker_cast.MIN_REGISTER_CONFIDENCE else "FAIL"
    else:
        conf_gate = "BLOCKED_BY_MARGIN"
    if margin_gate == "PASS" and conf_gate == "PASS":
        final = "AUTHORITATIVE_PASS"
    else:
        final = "FAIL_CLOSED"
    return {
        "raw_dominance": raw_dominance,
        "median_margin": median_margin,
        "margin_gate_result": margin_gate,
        "computed_confidence": confidence,
        "register_confidence_gate_result": conf_gate,
        "final_cast_result": final,
    }


def test_case_A_median_margin_0_079999_fails_closed(capsys):
    """CASE_A: median_margin=0.079999 -> FAIL_CLOSED (margin gate blocks)."""
    from services import subdub_multi_speaker_gender_onnx as multi_onnx
    # Construct 3 uniform cues whose per-cue margin = |0.5399995 - 0.4600005| / 1.0 = 0.079999
    cues = [
    ...
    ev = _compute_dual_gate_evidence(cues)
    print(f"\n=== CASE_A EVIDENCE ===")
    print(f"RAW_DOMINANCE={ev['raw_dominance']}")
    print(f"MEDIAN_MARGIN={ev['median_margin']}")
    print(f"MARGIN_GATE_RESULT={ev['margin_gate_result']}")
    print(f"COMPUTED_CONFIDENCE={ev['computed_confidence']}")
    print(f"REGISTER_CONFIDENCE_GATE_RESULT={ev['register_confidence_gate_result']}")
    print(f"FINAL_CAST_RESULT={ev['final_cast_result']}")
    assert ev["margin_gate_result"] == "FAIL"
    assert ev["final_cast_result"] == "FAIL_CLOSED"
    with pytest.raises(speaker_cast.AutoCastManualRequired):
        multi_onnx._aggregate_one_gender_result("spk_case_A", cues)


def test_case_B_median_margin_0_080000_fails_closed(capsys):
    """CASE_B: median_margin=0.080000 -> margin gate PASSES but confidence < 0.75 -> FAIL_CLOSED."""
    from services import subdub_multi_speaker_gender_onnx as multi_onnx
    # margin = |0.54 - 0.46| / 1.0 = 0.08 exactly; dominance = 3/3 = 1.0
    # confidence = (0.25 + 0.75*1.0) * min(1.0, 0.50 + 0.08) = 1.0 * 0.58 = 0.58 < 0.75
    cues = [
    ...
    print(f"REGISTER_CONFIDENCE_GATE_RESULT={ev['register_confidence_gate_result']}")
    print(f"FINAL_CAST_RESULT={ev['final_cast_result']}")
    assert ev["margin_gate_result"] == "PASS"
    assert ev["computed_confidence"] < speaker_cast.MIN_REGISTER_CONFIDENCE
    assert ev["final_cast_result"] == "FAIL_CLOSED"
    with pytest.raises(speaker_cast.AutoCastManualRequired):
        multi_onnx._aggregate_one_gender_result("spk_case_B", cues)


def test_case_C_median_margin_0_080001_fails_closed_when_combined_conf_lt_075(capsys):
    """CASE_C: median_margin=0.080001 -> margin gate PASSES but combined confidence < 0.75 -> FAIL_CLOSED."""
    from services import subdub_multi_speaker_gender_onnx as multi_onnx
    # margin = |0.5400005 - 0.4599995| / 1.0 ≈ 0.080001; dominance = 1.0
    # confidence ≈ 1.0 * (0.50 + 0.080001) = 0.580001 < 0.75
    cues = [
    ...
    print(f"FINAL_CAST_RESULT={ev['final_cast_result']}")
    assert ev["margin_gate_result"] == "PASS"
    assert ev["computed_confidence"] is not None
    assert ev["computed_confidence"] < speaker_cast.MIN_REGISTER_CONFIDENCE
    assert ev["final_cast_result"] == "FAIL_CLOSED"
    with pytest.raises(speaker_cast.AutoCastManualRequired):
        multi_onnx._aggregate_one_gender_result("spk_case_C", cues)


def test_case_D_strong_margin_and_strong_dominance_authoritative_pass(capsys):
    """CASE_D: strong margin (0.60) + strong dominance (1.0) -> AUTHORITATIVE_PASS."""
    from services import subdub_multi_speaker_gender_onnx as multi_onnx
    # margin = |0.80 - 0.20| / 1.0 = 0.60; dominance = 3/3 = 1.0
    # confidence = 1.0 * min(1.0, 0.50 + 0.60) = 1.0 * 1.0 = 1.0 >= 0.75
    cues = [
        {"start": 0.0, "end": 1.0, "male_score": 0.80, "female_score": 0.20},
        {"start": 1.0, "end": 2.0, "male_score": 0.80, "female_score": 0.20},
        {"start": 2.0, "end": 3.0, "male_score": 0.80, "female_score": 0.20},
    ...
    assert ev["computed_confidence"] is not None
    assert ev["computed_confidence"] >= speaker_cast.MIN_REGISTER_CONFIDENCE
    assert ev["final_cast_result"] == "AUTHORITATIVE_PASS"
    res, _ = multi_onnx._aggregate_one_gender_result("spk_case_D", cues)
    assert res["voice_gender"] == "male"
    assert res["voice_register"] == "low"
