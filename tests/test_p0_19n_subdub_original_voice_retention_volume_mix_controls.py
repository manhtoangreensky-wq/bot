...
def test_audio_mix_controls_only_show_for_dub_modes():
...
def test_audio_mix_default_preserves_existing_dub_only_behavior():
    ...
    assert mix["dubbed_voice_volume_percent"] == bot.SUBDUB_DUBBED_VOICE_DEFAULT_VOLUME_PERCENT
...
def test_audio_mix_keep_original_persists_percentages():
    ...
    assert "Giọng lồng tiếng" in lines
    assert "30%" in lines
    assert "150%" in lines


def test_audio_mix_keyboard_has_short_clean_public_labels():
    keyboard = bot.subdub_audio_mix_keyboard("vi", {"mode": bot.VIDEO_SUBTITLE_MODE_DUB})
    labels = [button.text for row in keyboard.inline_keyboard for button in row]
    public = " ".join(labels).lower()

    assert "Giữ âm gốc" in labels[0]
...
def test_confirm_keyboard_exposes_audio_mix_for_dub_modes_only():
    dub_callbacks = [
...
def test_render_video_mix_filter_uses_original_and_dub_volumes(monkeypatch):
    commands = []
