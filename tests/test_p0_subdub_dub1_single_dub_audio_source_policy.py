...
def test_target_language_english_uses_translated_text_only():
    policy = _policy({"target_language": "English", "translate_requested": "1"})
...
def test_missing_translation_never_renders_source_and_target_tts_together():
    policy = pipeline.resolve_subdub_dub_audio_policy(
        {"target_language": "English", "translate_requested": "1"},
        {
            ...
            "output_segments": [
                TRANSLATED[0],
                {**SOURCE[1], "translate_missing": True},
            ],
        },
    )
...
def test_pipeline_synthesizes_once_and_passes_explicit_original_audio_policy():
    calls = {"tts": [], "render": []}
