...
def test_help_main_menu_has_current_sections():
    ...
    assert guide_callbacks == [
        ...
        "menu|guide_video_ai",
        "menu|guide_audio",
        "menu|guide_subtitle_dub",
        "menu|guide_credits",
        "menu|guide_faq",
        "menu|main",
    ]
    assert "👨‍💼 Admin" not in guide_labels
    assert "🌐 Hub" not in guide_labels
    ...
    assert "🎧 Âm thanh" in _button_texts(vi_guide_keyboard)
...
def test_paid_addons_are_configured_before_tier():
    ...
    assert {"vfinal|subtitle", "vfinal|dub", "vfinal|combo", "vfinal|translate_sub"}.issubset(set(addon_callbacks))
