...
class AgentClaude:
    """Module Lập trình & Tester cấp cao kết nối trực tiếp API Anthropic thực tế"""
    @staticmethod
    async def generate_code(prompt: str) -> str:
        if not CLAUDE_API_KEY:
            return "⚠️ <b>[Claude Agent] Báo lỗi:</b> Thiếu cấu hình <code>CLAUDE_API_KEY</code> trên Railway. Đang chuyển giao tạm thời nhiệm vụ lập trình sang Gemini..."
        
        # Sửa đổi chuẩn xác cấu trúc dòng lệnh gọi API Claude Sonnet mượt mà nhất
        try:
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    return f"❌ Lỗi hệ thống Claude ({response.status_code}): {response.text}"
        except Exception as e:
            return f"❌ Lỗi đường truyền kết nối Claude Engine: {e}"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if action == "voice":
            ...
            code_reply = await AgentClaude.generate_code(data)
            await send_long_text(update, f"👨‍💻 <b>KẾT QUẢ PHÂN TÍCH CODE TỪ CLAUDE SONNET:</b>\n\n{code_reply}")
        elif action == "mmo":
...
async def cmd_status(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    status_text = (
        ...
        f"👨‍💻 <b>Claude Sonnet Core:</b> {'✅ Đã kết nối Key' if CLAUDE_API_KEY else '⏳ Chờ nạp Key'}\n"
